...
def mock_config_manager():
    ...
    mock.load_configuration.return_value = {
        "smithery_api_key": "test_api_key_123"
    }
    return mock
...
async def test_get_server_no_api_key(mock_config_manager):
    ...
    client_no_key = SmitheryClient(config_manager=mock_config_manager, config_name="default")
...
@pytest.fixture
def hub_manager(mock_console, mock_smithery_client, mock_config_manager, mock_main_client):
...
async def test_install_server_happy_path(hub_manager, mock_prompt_session, mock_smithery_client, mock_config_manager, mock_main_client):
    """Test the successful installation of a new server."""
    # Arrange
    server_name = "test/new-server"
    mock_prompt_session.prompt_async.return_value = server_name

    mock_config_manager.get_installed_servers.return_value = [] # No servers installed yet
    ...
    mock_server_details = {
        ...
        "connections": [{"type": "shttp", "configSchema": {"properties": {}}}] # No config needed
    }
...
async def test_install_server_already_installed(hub_manager, mock_prompt_session, mock_config_manager):
    """Test that installing an already installed server is handled gracefully."""
    # Arrange
    server_name = "test/existing-server"
    mock_prompt_session.prompt_async.return_value = server_name
    mock_config_manager.get_installed_servers.return_value = [{"qualifiedName": server_name}]
    ...
    hub_manager.console.print.assert_called_with(f"[yellow]Server '{server_name}' is already installed.[/yellow]")
    # Ensure we did not try to add the server again
    mock_config_manager.add_installed_server.assert_not_called()

async def test_uninstall_server_happy_path(hub_manager, mock_prompt_session, mock_config_manager, mock_main_client):
    """Test the successful uninstallation of a server."""
    # Arrange
    server_to_uninstall = {"qualifiedName": "test/to-uninstall", "displayName": "Uninstall Me"}
    mock_config_manager.get_installed_servers.return_value = [server_to_uninstall]
    mock_prompt_session.prompt_async.return_value = "1" # User chooses the first server
    ...
    mock_config_manager.remove_installed_server.assert_called_once_with("test/to-uninstall", "default")
    # 3. Reloaded servers
    mock_main_client.reload_servers.assert_called_once()

async def test_toggle_server_status(hub_manager, mock_prompt_session, mock_config_manager, mock_main_client):
    """Test toggling a server from enabled to disabled."""
    # Arrange
    server_to_toggle = {"qualifiedName": "test/toggle-me", "displayName": "Toggle Me", "enabled": True}
    mock_config_manager.get_installed_servers.return_value = [server_to_toggle]
    # Simulate loading the config for modification
    mock_config_manager.load_configuration.return_value = {"installed_servers": [server_to_toggle]}
    mock_prompt_session.prompt_async.return_value = "1"
