...
class ModelConfigManager:
    ...
    def display_parameter_explanations(self) -> None:
        ...
        header = Panel(
            Text.from_markup(
                "[bold]🔎 Model Parameter Reference Guide[/bold]", justify="center"
            ),
            expand=True,
            border_style="green",
        )
        ...
        combinations_table.add_row(
            "Factual, deterministic responses", "temperature=0.0-0.3, top_p=0.1-0.5"
        )
        combinations_table.add_row(
            "Balanced, natural responses", "temperature=0.7, top_p=0.9, typical_p=0.7"
        )
        combinations_table.add_row(
        ...
        combinations_table.add_row(
            "Reproducible outputs", "seed=42 (or any fixed integer value)"
        )
    ...
    def configure_model_interactive(
        self, clear_console_func: Optional[Callable] = None
    ) -> None:
        ...
        while True:
            ...
            self.console.print(
                Panel(
                    Text.from_markup(
                        "[bold]🪛 Model Configuration[/bold]", justify="center"
                    ),
                    expand=True,
                    border_style="green",
                )
            ...
            self.console.print(
                "Enter [bold orange3]number[/bold orange3] - [bold]Set a parameter[/bold] [dim](e.g., 1 for num_keep, 9 for temperature)[/dim]"
            )
            self.console.print(
                "Enter [bold][orange3]u[/orange3] + [orange3]number[/orange3][/bold] or [bold][magenta]usp[/magenta][/bold] - [bold]Unset a parameter[/bold] [dim](e.g., u1 to unset num_keep, usp to unset system_prompt)[/dim]"
            )
            self.console.print(
                "[bold]uall[/bold] - Unset all parameters [dim](use Ollama defaults)[/dim]"
            )
            self.console.print(
                "[bold]undo[/bold] - Restore original settings [dim](from before changes)[/dim]"
            )
            self.console.print(
                "[bold]h[/bold] or [bold]help[/bold] - Show Parameter Reference Guide"
            )
            self.console.print(
                "[bold]s[/bold] or [bold]save[/bold] - Save changes and return"
            )
            self.console.print(
                "[bold]q[/bold] or [bold]quit[/bold] - Cancel changes and return"
            )
            ...
            if selection == "uall":
                ...
                self.num_ctx = None
                result_message = (
                    "[green]All parameters unset (using Ollama defaults).[/green]"
                )
                result_style = "green"
                continue
            ...
            if selection.startswith("u") and len(selection) > 1:
                ...
                if param_to_unset == "sp" or param_to_unset == "system_prompt":
                    self.system_prompt = ""
                    result_message = (
                        "[green]system_prompt unset (using Ollama default).[/green]"
                    )
                    result_style = "green"
                    continue
                ...
                try:
                    ...
                    match param_num:
                        case 1:
                            self.num_keep = None
                            result_message = (
                                "[green]num_keep unset (using Ollama default).[/green]"
                            )
                            result_style = "green"
                        case 2:
                            self.seed = None
                            result_message = (
                                "[green]seed unset (using Ollama default).[/green]"
                            )
                            result_style = "green"
                        case 3:
                            self.num_predict = None
                        ...
                        case 4:
                            self.top_k = None
                            result_message = (
                                "[green]top_k unset (using Ollama default).[/green]"
                            )
                            result_style = "green"
                        case 5:
                            self.top_p = None
                            result_message = (
                                "[green]top_p unset (using Ollama default).[/green]"
                            )
                            result_style = "green"
                        case 6:
                            self.min_p = None
                            result_message = (
                                "[green]min_p unset (using Ollama default).[/green]"
                            )
                            result_style = "green"
                        case 7:
                            self.typical_p = None
                            result_message = (
                                "[green]typical_p unset (using Ollama default).[/green]"
                            )
                            result_style = "green"
                        case 8:
                            self.repeat_last_n = None
                        ...
                        case 13:
                            self.stop = None
                            result_message = (
                                "[green]stop unset (using Ollama default).[/green]"
                            )
                            result_style = "green"
                        case 14:
                            self.num_ctx = None
                            result_message = (
                                "[green]num_ctx unset (using Ollama default).[/green]"
                            )
                            result_style = "green"
                        case _:
            ...
            match selection:
                ...
                case "1":
                    try:
                        new_value = IntPrompt.ask(
                            "Keep Tokens (num_keep, 0 to context size)",
                            default=self.num_keep,
                        )
                        if new_value >= 0:
                            self.num_keep = new_value
                            result_message = (
                                f"[green]num_keep set to {new_value}.[/green]"
                            )
                            result_style = "green"
                        else:
                            result_message = (
                                "[red]num_keep must be a non-negative integer.[/red]"
                            )
                            result_style = "red"
                    except ValueError:
                ...
                case "2":
                    try:
                        new_value = IntPrompt.ask(
                            "Seed (integer for reproducible outputs, -1 for random)",
                            default=self.seed,
                        )
                        if new_value >= -1:
                            self.seed = new_value
                            ...
                            result_message = (
                                "[red]seed must be -1 or a non-negative integer.[/red]"
                            )
                            result_style = "red"
                    except ValueError:
                        result_message = "[red]Please enter a valid integer.[/red]"
                ...
                case "3":
                    try:
                        new_value = IntPrompt.ask(
                            "Max Tokens (num_predict, 0 to context size; 0 lets the server choose)",
                            default=self.num_predict,
                        )
                        if new_value >= 0:
                            self.num_predict = new_value
                            result_message = (
                                f"[green]num_predict set to {new_value}.[/green]"
                            )
                            result_style = "green"
                        else:
                            result_message = (
                                "[red]num_predict must be a non-negative integer.[/red]"
                            )
                            result_style = "red"
                    except ValueError:
                        result_message = "[red]Please enter a valid integer.[/red]"
                ...
                case "4":
                    try:
                        new_value = IntPrompt.ask(
                            "Top K (0 to disable)", default=self.top_k
                        )
                        if new_value >= 0:
                            ...
                            result_message = (
                                "[red]top_k must be a non-negative integer.[/red]"
                            )
                            result_style = "red"
                    except ValueError:
                        result_message = "[red]Please enter a valid integer.[/red]"
                ...
                case "5":
                    try:
                        new_value = FloatPrompt.ask(
                            "Top P (0.0-1.0)", default=self.top_p
                        )
                        if 0.0 <= new_value <= 1.0:
                            ...
                            result_message = (
                                "[red]top_p must be between 0.0 and 1.0.[/red]"
                            )
                            result_style = "red"
                    except ValueError:
                ...
                case "6":
                    try:
                        new_value = FloatPrompt.ask(
                            "Min P (0.0-1.0, 0 disables)", default=self.min_p
                        )
                        if 0.0 <= new_value <= 1.0:
                            ...
                            result_message = (
                                "[red]min_p must be between 0.0 and 1.0.[/red]"
                            )
                            result_style = "red"
                    except ValueError:
                        result_message = "[red]Please enter a valid number.[/red]"
                ...
                case "7":
                    try:
                        new_value = FloatPrompt.ask(
                            "Typical P (0.0-1.0, 1.0 disables)", default=self.typical_p
                        )
                        if 0.0 <= new_value <= 1.0:
                            self.typical_p = new_value
                            result_message = (
                                f"[green]typical_p set to {new_value}.[/green]"
                            )
                            result_style = "green"
                        else:
                            result_message = (
                                "[red]typical_p must be between 0.0 and 1.0.[/red]"
                            )
                            result_style = "red"
                    except ValueError:
                        result_message = "[red]Please enter a valid number.[/red]"
                ...
                case "8":
                    try:
                        new_value = IntPrompt.ask(
                            "Repeat Last N (-1 for entire context, 0 to disable)",
                            default=self.repeat_last_n,
                        )
                        if new_value >= -1:
                            self.repeat_last_n = new_value
                            result_message = (
                                f"[green]repeat_last_n set to {new_value}.[/green]"
                            )
                            result_style = "green"
                        else:
                ...
                case "9":
                    try:
                        new_value = FloatPrompt.ask(
                            "Temperature (≥ 0.0, lower = deterministic, higher = creative)",
                            default=self.temperature,
                        )
                        if new_value >= 0.0:
                            self.temperature = new_value
                            result_message = (
                                f"[green]temperature set to {new_value}.[/green]"
                            )
                            result_style = "green"
                        else:
                            result_message = (
                                "[red]temperature must be non-negative.[/red]"
                            )
                            result_style = "red"
                    except ValueError:
                        result_message = "[red]Please enter a valid number.[/red]"
                ...
                case "10":
                    try:
                        new_value = FloatPrompt.ask(
                            "Repeat Penalty (≥ 1.0, 1.0 = no penalty)",
                            default=self.repeat_penalty,
                        )
                        if new_value >= 1.0:
                            self.repeat_penalty = new_value
                            result_message = (
                                f"[green]repeat_penalty set to {new_value}.[/green]"
                            )
                            result_style = "green"
                        else:
                            result_message = (
                                "[red]repeat_penalty must be 1.0 or higher.[/red]"
                            )
                            result_style = "red"
                    except ValueError:
                        result_message = "[red]Please enter a valid number.[/red]"
                ...
                case "11":
                    try:
                        new_value = FloatPrompt.ask(
                            "Presence Penalty (0.0-2.0, 0 = no penalty)",
                            default=self.presence_penalty,
                        )
                        if 0.0 <= new_value <= 2.0:
                            self.presence_penalty = new_value
                            result_message = (
                                f"[green]presence_penalty set to {new_value}.[/green]"
                            )
                            result_style = "green"
                        else:
                ...
                case "12":
                    try:
                        new_value = FloatPrompt.ask(
                            "Frequency Penalty (0.0-2.0, 0 = no penalty)",
                            default=self.frequency_penalty,
                        )
                        if 0.0 <= new_value <= 2.0:
                            self.frequency_penalty = new_value
                            result_message = (
                                f"[green]frequency_penalty set to {new_value}.[/green]"
                            )
                            result_style = "green"
                        else:
                ...
                case "13":
                    ...
                    new_value = Prompt.ask(
                        "Stop Sequences (comma-separated)", default=default_val
                    )
                    if new_value and new_value.strip():
                        sequences = [
                            seq.strip() for seq in new_value.split(",") if seq.strip()
                        ]
                        if len(sequences) <= 8 and all(
                            len(seq) <= 255 for seq in sequences
                        ):
                            self.stop = sequences
                            result_message = (
                                f"[green]stop sequences set to {self.stop}.[/green]"
                            )
                            result_style = "green"
                        else:
                            result_message = (
                                "[red]Maximum 8 sequences, each ≤ 255 characters.[/red]"
                            )
                            result_style = "red"
                    else:
                        self.stop = []
                ...
                case "14":
                    try:
                        new_value = IntPrompt.ask(
                            "Context Size (num_ctx, size of context window)",
                            default=self.num_ctx,
                        )
                        if new_value >= 1:
                            self.num_ctx = new_value
                            result_message = (
                                f"[green]num_ctx set to {new_value}.[/green]"
                            )
                            result_style = "green"
                        else:
                            result_message = (
                                "[red]num_ctx must be a positive integer.[/red]"
                            )
                            result_style = "red"
                    except ValueError:
                        result_message = "[red]Please enter a valid integer.[/red]"
                ...
                case _:
                    result_message = (
                        "[red]Invalid selection. Please choose a valid option.[/red]"
                    )
                    result_style = "red"
