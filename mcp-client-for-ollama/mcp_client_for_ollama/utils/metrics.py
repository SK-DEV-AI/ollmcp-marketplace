...
def display_metrics(console, metrics):
    ...
    if total_duration > 0:
        metrics_lines.append(
            f"[cyan]total duration:[/cyan]       {total_duration:.9f}s"
        )
    if load_duration > 0:
        metrics_lines.append(
            f"[cyan]load duration:[/cyan]        {load_duration * 1000:.6f}ms"
        )
    if prompt_eval_count:
        metrics_lines.append(
            f"[cyan]prompt eval count:[/cyan]    {prompt_eval_count} token(s)"
        )
    if prompt_eval_duration > 0:
        metrics_lines.append(
            f"[cyan]prompt eval duration:[/cyan] {prompt_eval_duration * 1000:.6f}ms"
        )
    if eval_count:
        metrics_lines.append(
            f"[cyan]eval count:[/cyan]           {eval_count} token(s)"
        )
    if eval_duration > 0:
    ...
    if prompt_eval_count and prompt_eval_duration > 0:
        ...
        metrics_lines.append(
            f"[green]prompt eval rate:[/green]     {prompt_eval_rate:.2f} tokens/s"
        )

    if eval_count and eval_duration > 0:
        eval_rate = eval_count / eval_duration
        metrics_lines.append(
            f"[green]eval rate:[/green]            {eval_rate:.2f} tokens/s"
        )
