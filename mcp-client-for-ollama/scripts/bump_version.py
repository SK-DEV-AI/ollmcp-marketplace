...
def update_version_in_file(file_path, new_version):
    ...
    updated_content = version_pattern.sub(
        lambda m: f'{m.group(1)}"{new_version}"', content
    )

    # Also update any dependency on the main package (for the CLI package)
    dep_pattern = re.compile(r'("mcp-client-for-ollama==)([^"]+)"')
    updated_content = dep_pattern.sub(
        lambda m: f'{m.group(1)}{new_version}"', updated_content
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Bump version for MCP Client for Ollama packages"
    )
    parser.add_argument(
        "bump_type",
    ...
    parser.add_argument(
        "--version", help="Custom version to set when using the 'custom' bump type"
    )
    parser.add_argument(
        "--force",
        action="store_true",
    ...
    print(
        "4. Build and publish the packages will be done automatically by CI/CD pipeline."
    )
